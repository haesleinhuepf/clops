 ...
 * 6 2019
 */
@Plugin(type = CLIJMacroPlugin.class, name = "MOPS_maximumRectangle")
public class MaximumRectangle extends MopsBase implements OffersDocumentation {

    @Override
    public RandomAccessibleInterval run() {
        RandomAccessibleInterval result = (Img)ops.filter().max(asImg(args[0]), asImg(args[1]), asRectangle(args[2]));
