 ...
 * 6 2019
 */
@Plugin(type = CLIJMacroPlugin.class, name = "MOPS_meanRectangle")
public class MeanRectangle extends MopsBase implements OffersDocumentation {

    @Override
    public RandomAccessibleInterval run() {
        RandomAccessibleInterval result = (Img)ops.filter().mean(asImg(args[0]), asImg(args[1]), asRectangle(args[2]));
